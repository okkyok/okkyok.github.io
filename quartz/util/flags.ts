...
const flag = (code: FlagCode, name: string, part1: UnicodeCodePoint, part2: UnicodeCodePoint): FlagEmojiData => ({
  code,
  name,
  unicode: [part1, part2],
});
...
export const flagEmojiMap: Record<FlagCode, FlagEmojiData> = 
  flagData.reduce((acc, flag) => {
    acc[flag.code] = flag;
    return acc;
...
export function getAllFlagEmojis(): Record<string, string> {
  return Object.keys(flagEmojiMap).reduce((acc, code) => {
